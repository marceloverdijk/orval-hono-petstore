...
export const zValidator =
      ...
      in: HasUndefined<In> extends true
        ? {
            [K in Target]?: K extends 'json'
              ? In
              : HasUndefined<keyof ValidationTargetsWithResponse[K]> extends true
  ...
  async (c, next) => {
    if (target !== 'response') {
      const value = await zValidatorBase<T, keyof ValidationTargets, E, P, In, Out, I, V>(
        target,
        schema,
        hook,
      )(c, next);
      ...
      if (hook) {
        ...
        if (hookResult) {
          ...
          if ('response' in hookResult && hookResult.response instanceof Response) {
